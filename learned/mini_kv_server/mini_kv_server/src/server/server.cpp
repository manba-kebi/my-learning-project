...
namespace minikv::server {
	...
	void Server::run() {
		...
		while (true) {
			...
			auto client_ptr = std::make_shared<std::socket>(std::move(*client));
	...
	std::string Server::execut_command(const core::Command& command, bool& should_cloce) {
		switch(command.type){
		...
		case core::CommandType::Get: {
			const auto value = stroe_.get(command.key);
			if (!value.has_value()) {
				return "NOT_FOUND\n";
			}
			return "VALUE " + *value + '\n';
